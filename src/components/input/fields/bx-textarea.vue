...
const rows = ref(props?.options?.rows) || 3;
const text = ref("");
...
const inputActions = computed(() => {
    if (isArray(model.value)) return true;
...
function arrayError(index) {
    ...
    return valueError || characterError;
}
...
<template>
    <div class="bx-input" :class="[inputDisabled]">
        ...
        <template v-else>
            ...
            <div class="bx-input__inner" :class="[inputPrependIcon, inputPrependIconInner, inputAppendIcon, inputAppendIconInner, inputTextError, inputHasEnter]">
                <label class="bx-input__label" :class="[inputTextColor, inputTextError]" v-if="label">{{ label }}</label>
                ...
                <div class="bx-input__content" :class="[inputSimple, inputShadow]">
                    <textarea v-model="text" @input="emit" class="bx-input__field textarea" :class="[inputNotEmpty]" @focus="focus" @blur="blur" @keydown.enter.exact="addItem(text)" :rows="rows" :disabled="disabled" placeholder></textarea>
